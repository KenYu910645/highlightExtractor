...
from utils import cut_clip, burn_subtitles, write_clip_srt
...
def parse_highlight_md(md_path: str) -> list:
    ...
    for raw_index, body in pairs:
        ...
        # Extract fields — tolerate both * and - as list markers
        ...
        try:
            ...
            print(f"  WARNING: highlight_{index:02d} bad timestamp ({e}) — skipping")
            continue
...
# Duration clamping
# ═══════════════════════════════════════════════════════════════════════════════
def clamp_duration(start_sec: float, end_sec: float,
...
def main():
    ...
    parser.add_argument("--srt",             default=None,  help="Full video SRT for subtitle burning")
    parser.add_argument("--min-confidence",  type=float, default=0.0, help="Skip highlights below this confidence (default: 0.0)")
    parser.add_argument("--no-burn",         action="store_true",     help="Skip subtitle burning")
    parser.add_argument("--out-dir",         default=None,            help="Output directory (default: highlight/ next to video)")
    parser.add_argument("--min-dur",         type=int, default=10,    help="Minimum clip duration in seconds (default: 10)")
    parser.add_argument("--max-dur",         type=int, default=60,    help="Maximum clip duration in seconds (default: 60)")
    ...
    print(f"\n{'='*60}")
    print(f"  Post-processor")
    print(f"  Video      : {video_path.name}")
    print(f"  Highlights : {Path(args.highlights_md).name}")
    print(f"  Out dir    : {out_dir}")
    print(f"  Burn-in    : {'No' if args.no_burn else ('Yes' if args.srt else 'No (no SRT provided)')}")
    print(f"{'='*60}\n")
    ...
    # Load SRT segments if provided (for subtitle burning)
    segments = None
    if args.srt and not args.no_burn:
        segments = _parse_srt(args.srt)
        print(f"  ✓ Loaded SRT: {Path(args.srt).name} ({len(segments)} segments)")
    ...
    for i, h in enumerate(highlights, 1):
        start, end = clamp_duration(
            ...
            args.min_dur, args.max_dur, video_duration
        )
        duration = end - start
        name     = make_clip_name(i, start, h["reason"])
        ...
        if args.no_burn or segments is None:
            ...
            try:
                ...
                print(f"  WARNING: subtitle burn failed for clip {i:02d}, saved without burn")

        mm = int(start) // 60
        ss = int(start) % 60
        print(f"  ✓ [{i:02d}] {mm:02d}:{ss:02d}  {duration:.0f}s  conf={h['confidence']:.2f}  → {name}.mp4")
